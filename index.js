    ...
    for (let i = 0; i < 5; i++) {
        ...
        for (let dictWord of dictionary) {
            if (dictWord[i] == word[i])
                correctCount++;
            if (dictWord.includes(word[i]))
                presentCount++;
        }
        entropy += H(correctCount) + (1 / 5) * H(presentCount);
    }
